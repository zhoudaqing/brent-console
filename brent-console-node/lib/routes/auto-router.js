...
router.use('/', function(req, res) {
    ...
    // 获取用户登录信息
    try{
        qhClient.post('/action/SampleBasicAction/getWebContext',{
        },function(data){
            ctx.data.g_context = data.data;
            //if(config.debug){
                // 开发环境，之前qhee-webapp的地址直接连接到ORG上去，本地不启动tomcat
           //     ctx.data.g_context.context = 'http://www.qhee.org';
           //}
            ...
            for (var i = 0; i < myRouters.length; i++) {
                ...
                if (m) {
                    ...
                    if(myRouter.fn){
                        ...
                        // 2015-06-02，增加1个回调函数，支持在controller处理完后再回来继续执行pre预加载
                        if(preData) {
                            loginFilter.check(preData.loginType, ctx, function(){
