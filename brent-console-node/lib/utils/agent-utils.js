/**
 * Created by xiongxing on 2014/12/1.
 * 用于判断请求类型
 */
...
module.exports  = {
    ...
    isIos : function(request){
        ...
        if(!ua)
            return false;
		ua = ua.toLowerCase();
        if(ua.indexOf('iphone') != -1 || ua.indexOf('ipod') != -1 || ua.indexOf('itouch') != -1){
            return true;
        }else{
            return false;
        }
    ...
    isMobile : function(request){
        ...
        if(!ua)
            return false;
		ua = ua.toLowerCase();
        if(module.exports.isIos(request) || module.exports.isAndroid(request) || ua.indexOf('mobile ') != -1){
            return true;
        }else{
            return false;
        }
